            ...
            for metric_y_name in metric_names - {metric_x_name}:
                ...
                for environment_index, (environment, environment_df) in enumerate(df_grouped_by_environment):
                    ...
                    if len(dominant_points) > 0:
                        ...
                        dominant_points_x, dominant_points_y = max(xy, key=lambda x, y: (x_s*x, y_s*y))  # if no point is dominant, it means all points are aligned, so we can just take the maximum point (maximising both x and y)
