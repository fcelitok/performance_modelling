...
def generate_relations_o_and_re(run_output_folder, base_link_poses_file_path, ground_truth_file_path, output_file_name, metric_evaluator_exec_path, seconds=0.5, alpha=0.99, max_error=0.02, skip_ordered_recomputation=False):
    """
    ...
    # RE
    relations_re_file_path = path.join(run_output_folder, "Relations/Original/", output_file_name + "RE.relations")
    ...
    p1 = Popen(
        [metric_evaluator_exec_path, "-s", base_link_poses_file_path, "-r", relations_re_file_path, "-w",
         "{1.0,1.0,1.0,0.0,0.0,0.0}", "-e", "summaryT.error"])
    p1.wait()
    error_file = open("summaryT.error", "r")
    ...
    p1 = Popen([metric_evaluator_exec_path, "-s", base_link_poses_file_path, "-r", relations_re_file_path, "-w", "{0.0,0.0,0.0,1.0,1.0,1.0}", "-e", "summaryR.error"])
    p1.wait()
    error_file = open("summaryR.error", "r")
    content = error_file.readlines()
    words = content[1].split(", ")
    std = float(words[1])
    var = math.pow(std, 2)
    ...
    if not skip_ordered_recomputation:
        ordered_relations_file_path = path.join(run_output_folder, "Relations/Original/", output_file_name + "Ordered.relations")
...
def save_plot2(slam, gt, save):
    ...
    fig, ax = plt.subplots()
    ax.cla()
    ax.plot(x, y, 'r', x_gt, y_gt, 'g')
    print('SAVE')
    fig.savefig(save)
    plt.close(fig)


def generate_all(run_output_folder, dataset_name, metric_evaluator_exec_path, skip_ground_truth_conversion=False, skip_ordered_recomputation=False):
    """
    ...
    ground_truth_poses_path = path.join(run_output_folder, "relations")

    if not skip_ground_truth_conversion:
        if path.isfile(bag_file_path):
            write_ground_truth(bag_file_path, dataset_name)
        else:
            print("generate_all: bag file not found {}".format(bag_file_path))

    if path.isfile(base_link_poses_path):
        generate_relations_o_and_re(run_output_folder, base_link_poses_path, ground_truth_poses_path, dataset_name, metric_evaluator_exec_path, skip_ordered_recomputation=skip_ordered_recomputation)
...
if __name__ == '__main__':
    ...
    generate_all(args.folder_of_individual_output_run, args.skip_ground_truth_conversion)  # TODO update arguments or get rid of this
